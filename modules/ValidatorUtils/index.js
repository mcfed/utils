...
function fetchWhich(isGet, url, options) {
    return isGet ? index_1.FetchUtils.fetchGet(url, options) : index_1.FetchUtils.fetchPost(url, options);
}
function remote(rule, value, callback) {
    var _a, _b;
    var url = rule.url;
    var name = rule.field;
    var params = (typeof rule.params) === 'function' ? rule.params() : rule.params;
    params = __assign((_a = {}, _a[name] = value, _a), params);
    var isGet = rule.method ? rule.method.toUpperCase() === 'GET' : false;
    var options = rule.options ? __assign({}, rule.options) : {};
    options = { body: __assign((_b = {}, _b[name] = value, _b), params) };
    fetchWhich(isGet, url, options).then(function (res) {
        if (rule.callback) {
