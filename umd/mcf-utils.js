(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('buffer'), require('string_decoder'), require('stream'), require('util'), require('http'), require('url'), require('https'), require('zlib'), require('qs')) :
  typeof define === 'function' && define.amd ? define(['exports', 'buffer', 'string_decoder', 'stream', 'util', 'http', 'url', 'https', 'zlib', 'qs'], factory) :
  (factory((global.CRUD = {}),global.buffer,global.string_decoder,global.stream,global.util,global.http,global.url,global.https,global.zlib,global.qs));
}(this, (function (exports,buffer,string_decoder,stream,util,http,url,https,zlib,qs) { 'use strict';
  ...
  function fetchRequest(url$$1, options) {
    return fetch(url$$1, Object.assign({}, defaults, options)).then(function (res) {
  ...
  function fetchGet(url$$1, options) {
    if (options && _typeof(options.body) === 'object') {
      options.body = qs.stringify(processParams(options.body));
    }

    if (options && options.body && options.body !== "") {
      url$$1 = [url$$1, options.body].join("?");
    }

    options && delete options.body;
    return fetchRequest(url$$1, Object.assign({
      method: 'GET'
    }, options));
  }
  function fetchPost(url$$1, options) {
    if (options && _typeof(options.body) === 'object') {
